...
contract ForwardHook is BaseOverrideFee, SubScript {
    ...
    constructor(IPoolManager _poolManager, address _admin, address _underwriter, uint24 _defaultFee)
        BaseHook(_poolManager)
        SubScript(_underwriter, _defaultFee)
    {
    ...
    function setDefaultFee(PoolKey calldata key, uint24 fee) external onlyRole(UNDERWRITER_ROLE) {
        _setDefaultFee(fee);
    ...
    function _getFee(address agent, PoolKey calldata key, SwapParams calldata, bytes calldata)
        internal
        view
        override
        returns (uint24)
